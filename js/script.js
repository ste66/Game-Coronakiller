...
// add mousemove event listener to container control
...
// create array to save corona Object
const coronaArr = [];

// interval to generate corona div
setInterval(() => {
    // get container width
    ...
    // create interval to move the coronaDiv down by increasing the top property on them style
    setInterval(() =>{
...
//loop through coronaArr to change the top of coronaElement
coronaArr.forEach((element, idx) => {
    ...
    if ( element.top > containerHeight) {
        ...
        //delete corona element from the array
    ...
    container.addEventListener('click', e =>{
        ...
        const interval = setInterval(() => {
            ...
            if (bottom > containerHeight) {
                ...
                // call explode function to detect if bulletDiv touch the coronaDiv
    ...
    let score = 0;
    // explode function to detect if bulletDiv touch the coronaDiv
    function explode (bulletElement, interval) {

        // loop through coronaArr
        coronaArr.forEach((corona, idx) => {
            
            // check if coronaElement is in same area with the bulletElement
